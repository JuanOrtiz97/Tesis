...
\begin{tikzpicture}[
  ...
  \node[anchor=north west,problemaEtiqueta] at (0.1,10.85) {PLANTEAMIENTO DEL PROBLEMA};
  ...
  \node[anchor=north west,font=\fontfamily{phv}\selectfont\fontsize{17}{20}\selectfont] at (0.1,10.05)
    {Del diseño previsto\\\textbf{al desempeño verificable}};
  ...
  \node[anchor=north west,problemaTitulo] at (0.62,8.35) {Clima cálido-húmedo};
  \node[anchor=north west,problemaTexto] at (0.62,7.73)
    {Protección solar y\\ventilación por evaluar.};

  \node[anchor=north west,problemaNumero] at (0.10,5.65) {02};
  \node[anchor=north west,problemaTitulo] at (0.62,5.65) {Logística insular};
  \node[anchor=north west,problemaTexto] at (0.62,5.03)
    {Transporte y disponibilidad\\de materiales condicionados\\por el territorio.};

  \node[anchor=north west,problemaNumero] at (0.10,2.58) {03};
  \node[anchor=north west,problemaTitulo] at (0.62,2.58) {Envolvente propuesta};
  \node[anchor=north west,problemaTexto] at (0.62,1.96)
    {Estrategias existentes\\con efecto aún no cuantificado.};

  % Problema central, situado sobre la superficie semicircular.
  \node[anchor=north west,problemaEtiqueta,text=introBlue] at (5.57,6.22) {VACÍO DE EVIDENCIA};
  \node[anchor=north west,font=\fontfamily{phv}\selectfont\bfseries\fontsize{11.5}{14}\selectfont] at (5.57,5.72)
    {Desempeño\\sin verificación\\comparativa};
  \node[anchor=north west,problemaTexto] at (5.57,3.86)
    {Escenario base\\por comprobar.};
  \draw[->,introOrange,line width=1pt] (8.48,5.45) -- (9.03,5.45);

  % Evaluación requerida: mismo alcance del texto de la investigación.
  \node[anchor=north west,problemaEtiqueta,text=introBlue] at (9.20,8.16) {NECESIDAD DE EVALUACIÓN};
  ...
  \node[anchor=north west,problemaTexto] at (9.20,6.12)
    {Escenario base y alternativas\\en condiciones equivalentes.};
  ...
  \node[anchor=west,problemaTitulo] at (10.31,4.87) {Demanda energética};
  \node[anchor=west,problemaTexto] at (10.31,4.43) {Cuantificar requerimientos};
  \node[anchor=west,problemaTitulo] at (10.31,3.35) {Confort térmico};
  \node[anchor=west,problemaTexto] at (10.31,2.91) {Evaluar condiciones interiores};
  \node[anchor=west,problemaTitulo] at (10.31,1.83) {Sostenibilidad};
  \node[anchor=west,problemaTexto] at (10.31,1.39) {Valorar materiales y contexto};
  ...
  \node[anchor=south east,problemaEtiqueta] at (14.85,0.15)
    {Condiciones \quad / \quad Problema \quad / \quad Evaluación};
\end{tikzpicture}
\endgroup
